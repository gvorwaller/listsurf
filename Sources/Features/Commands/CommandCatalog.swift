...
enum CommandCatalog {
    ...
    static let addChild = command("addChild", "Add Child", "arrow.turn.down.right", binding: .init(key: .return, modifiers: [.command], display: "⌘ + Return"), helpText: "Starts a child item inside the selection.", surfaces: itemSurfaces.union(keyboardSurfaces))
    ...
    static let delete = command("delete", "Delete", "trash", binding: .init(key: .delete, modifiers: [.command], display: "⌘ + Delete"), helpText: "Deletes the selected items after confirmation.", surfaces: itemSurfaces.union(keyboardSurfaces))
