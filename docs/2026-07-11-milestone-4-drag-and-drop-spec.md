...
**Rev 3 (2026-07-12)**: implementer stop-and-report (correctly) caught that §5.1 step 3's `Array.move(fromOffsets:toOffset:)` is SwiftUI-only — unsatisfiable in Foundation-only Domain. Resolved: single-element move helper in Domain + a differential oracle test against SwiftUI's `Array.move` in the test target (which may import SwiftUI).
...
3. (Rev 3 — `Array.move(fromOffsets:toOffset:)` is SwiftUI-only and Domain imports Foundation only; the original step was unsatisfiable as written.) `var ids = visibleRows.map(\.id)`, then apply a private Domain helper `movedSingleElement(_:from:to:)` implementing exactly the single-element case of `.onMove` destination semantics: `let el = ids.remove(at: sourceIndex); ids.insert(el, at: destination > sourceIndex ? destination - 1 : destination)`. The off-by-one risk the original step guarded against is retired by a **differential oracle test** instead: `TreeEngineReorderTests` MAY `import SwiftUI` (test targets are not bound by Domain's import rule) and must assert the helper's output equals `Array.move(fromOffsets: IndexSet(integer: s), toOffset: d)` for EVERY `(s, d)` pair over an 8-element array (`s in 0..<8`, `d in 0...8`) — Apple's implementation remains the semantic truth, enforced by test rather than by runtime reuse.
