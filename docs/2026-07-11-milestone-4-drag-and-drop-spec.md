...
**D12 — Drag payload: the view-local `draggedItemID: UUID?` is the sole identity for local drops — UUID, never index.** The `NSItemProvider` returned from `.onDrag` carries the item's *title* as plain text (`NSItemProvider(object: item.title as NSString)`) so dragging a row into another app pastes something human-useful; **it is never parsed by our drop code.** `validateDrop`/`dropUpdated` require `draggedItemID != nil` and the ID present in `store.items`; otherwise `.forbidden`. This automatically rejects external drags, cross-list drags from another window, and stale sessions. (Cross-list drag is a non-goal, §13.)
...
**D16 — Spring-loaded expansion**: hovering a *nest-depth* proposal (`indicatorDepth == G.depth + 1`) over a collapsed parent for **0.7 s** expands it (Finder-like dwell). Implemented as a view-local `Task.sleep` started when such a proposal first appears for a row, cancelled when the proposal moves away or the drop completes; on firing it calls `store.setExpanded(id, true)`.
...
**D23 — Drag-in-progress state is view-local `@State` in `OutlineEditorView`, not `ListStore`.** The full set: `draggedItemID: UUID?`, `dropProposal: OutlineDropTarget?`, `dragSession: (startDepth: Int, xRef: CGFloat)?`, the spring-load `Task`. Justification against the store-owned rule: store-owned presentation state exists so *multiple surfaces* (toolbar, menus, commands) read one truth (`ListStore.swift:33-34`); drag hover state has exactly one consumer (the editor's rows), must die with the view, and must never gate commands. The *commit* goes through store API (`performDrop`), preserving the explicit commit lifecycle — this is the same split as view-local `editingText` draft vs store-owned `editingItemID`. A mid-drag mode switch destroys the view and the state with it; nothing lingers.
...
4. Candidate depths, ordered by distance from `clamp(requestedDepth, minDepth...maxDepth)` (nearest-first, preferring the shallower on ties): for each depth `d`:
   ...
   - `d ≤ G.depth`: walk G's ancestor chain (`ancestorIDs(of:in:)`, `TreeEngine.swift:212-224`, prepend G itself) to find `A`, the ancestor-or-self of G whose depth == `d` (compute depths from `visibleRows`/parent chain, not by trusting indices). Parent = `A.parentID`. Illegal if parent is in `forbidden` (A itself being the dragged item is fine only when… it can't be: dragged item's own row hosts gaps, and A == draggedID means the gap is inside/below the dragged row — parent would be dragged's parent; legal, yields an identity-adjacent move; allow it, the engine no-op guard handles identity). Slot = index of `A` within `parentID`'s sibling group **with the dragged item excluded**, + 1.
...
3. A dragged parent's subtree arrives intact — only the dragged item's `parentID`/`position`/`updatedAt` change.
