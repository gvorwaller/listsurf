...
Hard rule: every phase ends with the full suite green, both xctestplans green, the manual gate executed on real macOS (+ iOS sim where noted), a devlog entry, td closure. No phase starts until the previous gate passes. Phases 2 and 4 add/delete files → run `xcodegen` in those phases only.
...
`CommandCatalog.swift` (§3); `ListsurfCommands`/`ItemActionsMenu` refactored to reference it; Help keyboard section generated from `macKeyboardHelp`; `⌘?` and `⌘,` entries included; **help sections are platform-scoped**: the keyboard section renders only on macOS, and the iOS help carries a single line — "Hardware keyboard attached? The Mac shortcuts work on iPad and iPhone too." — so touch users never read keys they cannot press; ellipsis `selectsTargetOnAct`; empty-selection context-menu additions; `CommandCatalogTests`.

**Gate M4 (manual)**: the consistency audit — read every Help line while performing it in the app; open every menu surface (menu bar Item/View, row right-click, row ellipsis, toolbar actions, empty-area right-click) and diff against the Item menu; press every advertised shortcut once. Any mismatch is a phase failure. Devlog closes the overhaul; TestFlight build for dogfooding.
